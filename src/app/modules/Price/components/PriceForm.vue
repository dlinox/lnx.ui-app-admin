        ...
        <n-row :gutter="16">
          ...
          <n-col span="8">
            <n-form-item
              ...
              :feedback="formErrors?.enrollmentPrice"
            >
              <n-input-number
                v-model:value="form.enrollmentPrice"
                :status="
                  formErrors?.enrollmentPrice != undefined ? 'error' : ''
                "
                @update:modelValue="
                  formErrors != null
                    ? (formErrors.enrollmentPrice = null)
                    : () => {}
                "
              />
            </n-form-item>
...
import {
  type PriceDataTableItemDTO,
  type PriceFormErrorsDTO,
} from "@/app/modules/Price/types/Price.types";

import {
  _getPriceInitValues,
  _getPriceRules,
} from "@/app/modules/Price/configs/form.configs";
...
const props = defineProps<{
  item: PriceDataTableItemDTO | null;
  modelValue: boolean;
...
const form = ref<PriceDataTableItemDTO>(_getPriceInitValues());
const formErrors = ref<PriceFormErrorsDTO | null>(null);
...
const handleSubmit = async () => {
  ...
  if (formRef.value) {
    ...
    if (valid) {
      ...
      if (!response.status) {
        formErrors.value = response.data as PriceFormErrorsDTO;
      } else {
        emit("success");
