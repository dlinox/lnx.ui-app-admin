<template>
    <n-dropdown trigger="click" :options="options" :show-arrow="true" @select="handleSelect">
        <n-button :render-icon="renderIcon('magicpen')">
            Opt.
        </n-button>
    </n-dropdown>
</template>
...
const emit = defineEmits(["changeGroup", "reserveEnrollment", "cancelEnrollment", "resetEnrollment"]);
...
const options = computed(() => [
    ...
    },
    {
        ...
        disabled: props.item?.enrollmentStatus !== 'CANCELADO' && props.item?.enrollmentStatus !== 'RESERVADO'
    }
]);
</script>