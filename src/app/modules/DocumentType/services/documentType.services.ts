...
export const _storeItem = async (
  ...
  try {
    let reponse = await http().post("/document-type", request);
    return {
      status: true,
...
export const _updateItem = async (
  ...
  try {
    let reponse = await http().put("/document-type", request);
    return {
      status: true,
      data: reponse.data.data as DocumentTypeFormDTO,
    };
...
export const _deleteItem = async (
  ...
  try {
    await http().delete("/document-type", { data: { id: request.id } });
    return true;
  } catch (error) {
    return false;
  }
...
> => {
  try {
    let response = await http().get("/document-type/items/for-select");
