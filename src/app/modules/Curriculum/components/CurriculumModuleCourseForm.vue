...
import {
  type CurriculumModuleCourseDTO,
  type CurriculumModuleCourseFormDTO,
  type CurriculumModuleCourseFormErrorsDTO,
  type CurriculumModuleDTO,
} from "@/app/modules/Curriculum/types/Curriculum.types";

import {
  _getCurriculumModuleCourseInitValues,
...
const curriculumModule =
  inject<Ref<CurriculumModuleDTO | undefined>>("curriculumModule");

const emit = defineEmits(["update:modelValue", "success"]);

const props = defineProps<{
  item: CurriculumModuleCourseDTO | null;
  modelValue: boolean;
  selectables: any;
}>();
...
const form = ref<CurriculumModuleCourseFormDTO>(
  _getCurriculumModuleCourseInitValues()
);
const formErrors = ref<CurriculumModuleCourseFormErrorsDTO | null>(null);
...
const handleSubmit = async () => {
  if (formRef.value) {
    ...
    if (valid) {
      loading.value = true;
      form.value.curriculumModuleId = curriculumModule!.value!.id;
      if (form.value.id) {
      ...
      if (!response.status) {
        formErrors.value = response.data as CurriculumModuleCourseFormErrorsDTO;
      } else {
        emit("success");
...
const getItemForm = async () => {
  if (props.item?.id) {
    ...
    form.value = _getCurriculumModuleCourseInitValues();
  }
};
